    ...
    try:
        ...
        if search_name:
            ...
            if detect_suspicious_patterns(search_name):
                log_security_event('suspicious_search_attempt', {
                    'user_id': current_user.id,
        ...
        if family_member_id is not None:
            if family_member_id == 0:
                ...
                if family_member and family_member in current_user.family_members:
                    ...
                    log_security_event('unauthorized_family_access_attempt', {
                        'user_id': current_user.id,
...
def create_record():
    ...
    if form.validate_on_submit():
        try:
            ...
            chief_complaint = sanitize_html(form.chief_complaint.data) if form.chief_complaint.data else None
            doctor = sanitize_html(form.doctor.data) if form.doctor.data else None
            investigations = sanitize_html(form.investigations.data) if form.investigations.data else None
            diagnosis = sanitize_html(form.diagnosis.data) if form.diagnosis.data else None
            prescription = sanitize_html(form.prescription.data) if form.prescription.data else None
            notes = sanitize_html(form.notes.data) if form.notes.data else None
            review_followup = sanitize_html(form.review_followup.data) if form.review_followup.data else None
            ...
            if form.family_member.data == 0:
                ...
                if family_member and family_member in current_user.family_members:
                    ...
                    log_security_event('invalid_family_member_assignment', {
                        'user_id': current_user.id,
            ...
            log_security_event('health_record_created', {
                'user_id': current_user.id,
                'record_id': record.id,
            ...
            flash('An error occurred while creating the health record. Please try again.', 'danger')
...
def view_record(record_id):
    ...
    try:
        ...
        log_security_event('health_record_accessed', {
            'user_id': current_user.id,
            'record_id': record_id
        })
    ...
    try:
        ...
        if form.validate_on_submit():
            try:
                ...
                record.chief_complaint = sanitize_html(form.chief_complaint.data) if form.chief_complaint.data else None
                record.doctor = sanitize_html(form.doctor.data) if form.doctor.data else None
                record.investigations = sanitize_html(form.investigations.data) if form.investigations.data else None
                record.diagnosis = sanitize_html(form.diagnosis.data) if form.diagnosis.data else None
                record.prescription = sanitize_html(form.prescription.data) if form.prescription.data else None
                record.notes = sanitize_html(form.notes.data) if form.notes.data else None
                record.review_followup = sanitize_html(form.review_followup.data) if form.review_followup.data else None
                ...
                flash('An error occurred while updating the health record. Please try again.', 'danger')
...
def delete_record(record_id):
    ...
    try:
        ...
        for doc in documents:
            try:
                if os.path.exists(doc.file_path):
                    ...
                    current_app.logger.warning(f"File not found on disk: {doc.file_path}")
        ...
        log_security_event('health_record_deleted', {
            'user_id': current_user.id,
            'record_id': record_id,
...
def add_family_member():
    ...
    if form.validate_on_submit():
        try:
            ...
            current_medications = sanitize_html(form.current_medications.data) if form.current_medications.data else None
            allergies = sanitize_html(form.allergies.data) if form.allergies.data else None
            chronic_conditions = sanitize_html(form.chronic_conditions.data) if form.chronic_conditions.data else None
            notes = sanitize_html(form.notes.data) if form.notes.data else None

            # Validate names contain no suspicious patterns
            if detect_suspicious_patterns(first_name) or detect_suspicious_patterns(last_name):
                log_security_event('suspicious_family_member_name', {
                    'user_id': current_user.id,
                    'first_name': first_name,
                    'last_name': last_name
                })
                flash('Invalid characters in name fields', 'danger')
                return render_template('records/family_form.html', title='Add Family Member', form=form)
            ...
            log_security_event('family_member_created', {
                'user_id': current_user.id,
                'family_member_id': family_member.id,
            ...
            flash('An error occurred while adding the family member. Please try again.', 'danger')
...
def serve_upload(record_id, filename):
    ...
    try:
        ...
        if not filename or '..' in filename or '/' in filename or '\\' in filename:
            log_security_event('file_access_path_traversal_attempt', {
                'user_id': current_user.id,
                'filename': filename,
                'record_id': record_id
            })
        ...
        if not real_file_path.startswith(real_upload_dir):
            log_security_event('file_access_directory_traversal', {
                'user_id': current_user.id,
                'record_id': record_id,
                'filename': filename,
