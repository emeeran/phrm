...
if not os.getenv("HUGGINGFACE_ACCESS_TOKEN"):
    ...
    print(
        "   The application will attempt to use local MedGemma or fallback to GROQ/DEEPSEEK."
    )
else:
...
if (
    not os.getenv("HUGGINGFACE_ACCESS_TOKEN")
...
elif not os.getenv("HUGGINGFACE_ACCESS_TOKEN") and (
    not os.getenv("GROQ_API_KEY") or not os.getenv("DEEPSEEK_API_KEY")
):
    print(
        "⚠️ WARNING: Primary AI (MedGemma via Hugging Face) is not configured, and one or more fallback AIs are also not configured."
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run the Personal Health Record Manager"
    )
    parser.add_argument(
        "--port", type=int, default=5000, help="Port to run the application on"
    )
    parser.add_argument(
        "--host", type=str, default="0.0.0.0", help="Host to run the application on"
    )
    ...
    with app.app_context():
        if not os.path.exists(
            os.path.join(os.path.dirname(__file__), "app", "phrm.db")
        ):
            db.create_all()
